...
-define(DBG(Msg, Args), io:format(Msg, Args)).
-else.
-define(DBG(Msg), ok).
-define(DBG(Msg, Args), ok).
-endif.
...
%%------------------------------------------------------------------------------
init() ->
    Nif = filename:join([code:priv_dir(md2), erlang:atom_to_list(?MODULE)]),
    case erlang:load_nif(Nif, 0) of
        ok ->
            ?DBG("NIF loaded successfully~n");
        {error, _Reason} ->
            ?DBG("failed to load NIF using fallback~n")
    end.
...
hash_empty_test() ->
    ...
    Bin = <<>>,
    ?assertEqual(Digest, to_string(loop(Bin))),
    ?assertEqual(Digest, to_string(hash(Bin))).
